...
func (kv *KeyValueEmbd) detectEmbeddingDim() int {
	var length int
	err := kv.db.QueryRow(
		`SELECT length(embedding) FROM ` + vectorTableName +
...
func (kv *KeyValueEmbd) MigrateVectorIndex() error {
	...
	// 2. Backfill the vector column from the raw embedding blob. libSQL
	// accepts the little-endian float32 layout as-is, so no re-encoding is
	// needed.
	res, err := kv.db.Exec(fmt.Sprintf(
		`UPDATE %s SET %s = embedding
		 WHERE embedding IS NOT NULL AND %s IS NULL`,
		vectorTableName, vectorColumnName, vectorColumnName,
	))
	if err != nil {
...
func (kv *KeyValueEmbd) countEmbeddings() (int, error) {
	var n int
	err := kv.db.QueryRow(
		`SELECT COUNT(*) FROM ` + vectorTableName + ` WHERE embedding IS NOT NULL`,
	).Scan(&n)
	return n, err
}
