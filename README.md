...
Default metadata values and RFC3339 timestamps are written by the package code
(`Set`, `SetInfo`, `SetWithEmbedding`) rather than by database defaults. Older
databases created before v0.2.0 may have stricter `NOT NULL` / `DEFAULT`
constraints; they remain compatible because `CREATE TABLE IF NOT EXISTS` does
